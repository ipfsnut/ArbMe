...
import { executeV4Swap, createPoolKey, getSwapDirection, } from './uniswap-v4-swap';
...
async function testTinySwap() {
    ...
    console.log('');
    // Create pool key
    const poolKey = createPoolKey(POOLS.ARBME_WETH.token0, POOLS.ARBME_WETH.token1, POOLS.ARBME_WETH.fee, POOLS.ARBME_WETH.tickSpacing);
    ...
    console.log('');
    const result = await executeV4Swap(walletClient, publicClient, swapParams, 120);
    if (result.success) {
