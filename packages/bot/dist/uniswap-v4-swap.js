 ...
 */
import { encodeAbiParameters, parseAbiParameters, encodePacked, formatUnits, } from 'viem';
// ═══════════════════════════════════════════════════════════════════════════════
// CONTRACTS
// ═══════════════════════════════════════════════════════════════════════════════
 ...
 */
export async function approveTokenForPermit2(client, publicClient, token, amount = BigInt('0xffffffffffffffffffffffffffffffffffffffff')) {
 ...
 */
export async function approveRouterViaPermit2(client, publicClient, token, amount = BigInt('0xffffffffffffffffffffffffffffffffffffffff'), // Max uint160
...
export function encodeV4Swap(params) {
    ...
    actionParams.push(encodeAbiParameters(parseAbiParameters('(address,address,uint24,int24,address),bool,uint128,uint128,uint160,bytes'), [
        [
            params.poolKey.currency0,
            params.poolKey.currency1,
            params.poolKey.fee,
    ...
    actionParams.push(encodeAbiParameters(parseAbiParameters('address,uint256'), [currencyIn, params.amountIn]));
    // 3. TAKE_ALL params (output currency and minimum)
    actionParams.push(encodeAbiParameters(parseAbiParameters('address,uint256'), [currencyOut, params.amountOutMinimum]));
    // Combine actions and params
    const input = encodeAbiParameters(parseAbiParameters('bytes,bytes[]'), [actions, actionParams]);
    // Command for V4_SWAP
 ...
 */
export async function executeV4Swap(client, publicClient, params) {
    try {
        const account = client.account;
        console.log('\n🔄 EXECUTING V4 SWAP');
        console.log('═══════════════════════════════════════════════════════════════');
