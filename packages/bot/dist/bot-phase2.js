...
import { TradeExecutor, TOKENS } from './trade-executor';
import { getSwapQuote, POOLS } from './uniswap-v4';
import { ProfitCalculator, formatProfitAnalysis } from './profit-calculator';
import { createPoolKey } from './uniswap-v4-swap';
import { MIN_RESERVES, POSITION_SIZE_PERCENT, MAX_TRADE_USD, calculateAvailableBalance, } from './portfolio-config';
...
// ═══════════════════════════════════════════════════════════════════════════════
async function detectOpportunities(data) {
    const opportunities = [];
...
async function executeTrade(opportunity, prices) {
    ...
    try {
        ...
        if (opportunity.pool.pair.includes('ARBME / WETH')) {
            ...
            poolKey = createPoolKey(TOKENS.ARBME, TOKENS.WETH, 30000, // 3% fee
            200);
        }
        else if (opportunity.pool.pair.includes('CLANKER / ARBME')) {
            poolId = POOLS.CLANKER_ARBME;
            poolKey = createPoolKey(TOKENS.ARBME, '0x1bc0c42215582d5a085795f4badbac3ff36d1bcb', // CLANKER
            30000, // 3% fee
            200);
        }
        else {
        ...
        // Get quote
        console.log('\n📊 Getting swap quote...');
        const quote = await getSwapQuote(publicClient, poolId, amountIn, opportunity.tokenIn, opportunity.tokenOut, 18, // decimals in
        18, // decimals out
        CONFIG.SLIPPAGE_TOLERANCE, 3);
        ...
        const profitAnalysis = calculator.calculateTradeProfit(opportunity.tokenInSymbol, opportunity.tokenOutSymbol, amountIn, quote.expectedAmountOut, quote.minAmountOut, gasCost, 3);
        console.log('\n💰 PROFIT ANALYSIS:');
        ...
        // Execute trade
        console.log('\n🚀 Executing trade...');
        const result = await executor.executeTrade(opportunity.tokenIn, opportunity.tokenOut, amountIn, poolKey);
        if (result.success) {
            console.log(`\n✅ TRADE SUCCESSFUL!`);
...
// DISPLAY
// ═══════════════════════════════════════════════════════════════════════════════
function displayState(data, opportunities) {
    console.clear();
    console.log('═══════════════════════════════════════════════════════════════');
