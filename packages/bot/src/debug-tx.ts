...
import { createPublicClient, http, parseAbiParameters, decodeErrorResult } from 'viem';
...
async function debugTransaction(txHash: `0x${string}`) {
...
debugTransaction(txHash as `0x${string}`).catch(console.error);
