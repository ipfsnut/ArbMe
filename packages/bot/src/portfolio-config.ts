...
/**
 * When multiple opportunities exist, prioritize trades that:
 * 1. Move us closer to target allocation
 * 2. Have highest profit
 * 3. Have lowest risk
 */
export interface TradeScore {
  profitUsd: number;
  rebalanceBonus: number; // Bonus if this trade improves portfolio balance
  totalScore: number;
}
...
export function calculateAvailableBalance(
  totalBalance: bigint,
  reserve: bigint,
  positionSize: number = POSITION_SIZE_PERCENT
): bigint {
  if (totalBalance <= reserve) return 0n;
 ...
 */
export function calculatePortfolioImbalance(
  ethUsd: number,
  arbmeUsd: number,
  clankerUsd: number,
  wethUsd: number
