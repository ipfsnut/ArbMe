...
async function analyzeSuccessfulSwap(txHash: `0x${string}`) {
  ...
  try {
    ...
    if (decoded[1].length > 0) {
      ...
      if (decoded[0] === '0x10') {
        ...
        try {
          const v4Input = decodeAbiParameters(
            parseAbiParameters('bytes,bytes[]'),
            decoded[1][0] as `0x${string}`
          );
...
analyzeSuccessfulSwap(successfulTx as `0x${string}`).catch(console.error);
