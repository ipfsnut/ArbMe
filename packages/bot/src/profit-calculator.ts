...
export function getTokenPrice(symbol: string, prices: PriceFeed): number {
...
export class ProfitCalculator {
  constructor(private prices: PriceFeed) {}
  ...
  calculateArbitrageProfit(
    // First leg: Buy ARBME
    buyWithSymbol: string, // WETH or CLANKER
    buyAmountIn: bigint,
    buyExpectedOut: bigint,
    buyMinOut: bigint,
    buyGasCost: bigint,
    buyFeePercent: number,

    // Second leg: Sell ARBME
    sellForSymbol: string, // WETH or CLANKER
    sellExpectedOut: bigint,
    sellMinOut: bigint,
    sellGasCost: bigint,
    sellFeePercent: number,
  ): ProfitAnalysis {
    // Calculate first leg
    const leg1 = this.calculateTradeProfit(
      buyWithSymbol,
      'ARBME',
      buyAmountIn,
      buyExpectedOut,
    ...
    const leg2 = this.calculateTradeProfit(
      'ARBME',
      sellForSymbol,
      leg1.minAmountOut, // Use worst-case from leg 1
      sellExpectedOut,
      sellMinOut,
      sellGasCost,
      sellFeePercent,
   ...
   */
  updatePrices(newPrices: Partial<PriceFeed>) {
 ...
 */
export function calculateMinProfitableAmount(
  gasCostUsd: number,
  swapFeePercent: number,
  minProfitUsd: number,
