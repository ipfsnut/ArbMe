...
import { createPublicClient, http, encodeFunctionData, decodeFunctionResult, parseAbiParameters, decodeAbiParameters } from 'viem';
...
async function getRawPosition() {
  ...
  const decoded = decodeAbiParameters(
    parseAbiParameters('(address,address,uint24,int24,address),bytes32'),
    result.data
  );
