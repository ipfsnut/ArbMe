...
import { fetchPoolsForToken } from '../lib/gecko.js';
import { generateMarkdownReport, generateJsonReport, generateEmptyReport, } from '../lib/report.js';
const WETH_SYMBOLS = ['weth', 'eth'];
const USDC_SYMBOLS = ['usdc', 'usd coin'];
// ═══════════════════════════════════════════════════════════════════════════════
// Analysis helpers
// ═══════════════════════════════════════════════════════════════════════════════
function computeSpread(pools) {
...
function computeLiquidity(pools, totalTvl) {
...
function computeFeeTiers(pools) {
    const tiers = {};
...
function computeDexBreakdown(pools) {
    const breakdown = {};
    for (const p of pools) {
        const key = p.dex ? `${p.dex} (${p.version})` : p.version;
        if (!breakdown[key])
            breakdown[key] = { count: 0, tvl: 0 };
        breakdown[key].count++;
        breakdown[key].tvl += p.tvl || 0;
    }
    return breakdown;
}
function computeRouting(pools, tokenAddress) {
    ...
    for (const p of pools) {
        ...
        const quoteSymbol = p.baseTokenAddress.toLowerCase() === normalizedToken
            ? p.quoteTokenSymbol
            : p.baseTokenSymbol;
        if (quoteSymbol) {
            quoteTokens.add(quoteSymbol);
            const lower = quoteSymbol.toLowerCase();
            if (WETH_SYMBOLS.includes(lower))
                hasWethPair = true;
            if (USDC_SYMBOLS.includes(lower))
                hasUsdcPair = true;
        }
        // Track best entry pool (highest TVL with WETH or USDC)
        const isStable = quoteSymbol &&
...
function computeHealthScore(totalTvl, totalPools, spread, routing, liquidity) {
    let score = 0;
    // TVL score (0-30 points)
    if (totalTvl >= 1_000_000)
        score += 30;
    else if (totalTvl >= 100_000)
        score += 25;
    else if (totalTvl >= 10_000)
        score += 15;
    else if (totalTvl >= 1_000)
        score += 8;
    else if (totalTvl > 0)
        score += 3;
    ...
    if (totalPools >= 10)
        score += 15;
    else if (totalPools >= 5)
        score += 12;
    else if (totalPools >= 3)
        score += 8;
    else if (totalPools >= 1)
        score += 4;
    // Spread tightness (0-20 points)
    if (spread.spreadPct === 0 && totalPools <= 1)
        score += 10; // only 1 pool, N/A
    else if (spread.spreadPct < 1)
        score += 20;
    else if (spread.spreadPct < 3)
        score += 15;
    else if (spread.spreadPct < 5)
        score += 10;
    else if (spread.spreadPct < 10)
        score += 5;
    // Routing (0-20 points)
    ...
    if (routing.hasUsdcPair)
        score += 5;
    if (routing.uniqueQuoteTokens.length >= 3)
        score += 5;
    else if (routing.uniqueQuoteTokens.length >= 2)
        score += 3;
    // Concentration risk (0-15 points) — lower is better
    if (liquidity.topPoolPct < 50)
        score += 15;
    else if (liquidity.topPoolPct < 70)
        score += 10;
    else if (liquidity.topPoolPct < 90)
        score += 5;
...
function generateRecommendations(analysis) {
    ...
    if (!routing.hasWethPair) {
        recs.push('Create a WETH pair — this is the primary on-ramp for most traders on Base.');
    }
    if (!routing.hasUsdcPair) {
        recs.push('Consider adding a USDC pair for stablecoin access and to improve aggregator routing.');
    }
    if (totalTvl < 10_000) {
        recs.push(`Total TVL is low (${fmtUsd(totalTvl)}). Adding liquidity would improve price stability and attract more volume.`);
    }
    if (spread.spreadPct > 5) {
        recs.push(`Price spread is ${spread.spreadPct.toFixed(1)}% across pools — indicates stale liquidity or mismatched prices. Consider rebalancing or removing low-liquidity pools.`);
    }
    else if (spread.spreadPct > 2) {
        recs.push(`Moderate spread of ${spread.spreadPct.toFixed(1)}%. Monitor for arbitrage opportunities.`);
    }
    if (liquidity.topPoolPct > 80) {
        recs.push(`${liquidity.topPoolPct.toFixed(0)}% of TVL is concentrated in one pool. Diversifying liquidity across pools reduces single-point-of-failure risk.`);
    }
    if (routing.uniqueQuoteTokens.length < 2) {
        recs.push('Only 1 quote token paired. Adding more pairs (WETH, USDC) improves aggregator discoverability.');
    }
    if (totalPools < 3) {
        recs.push('Few pools detected. Deploying pools across multiple DEXes (Uniswap, Aerodrome) broadens market access.');
    }
...
// Command handler
// ═══════════════════════════════════════════════════════════════════════════════
export async function poolHealth(token, options) {
    ...
    // Step 2: Fetch pools
    if (verbose)
        console.error('  Fetching pools from GeckoTerminal...');
    ...
    const healthScore = computeHealthScore(result.totalTvl, result.totalPools, spread, routing, liquidity);
    const analysis = {
        token: {
            address: token,
            symbol: meta.symbol,
