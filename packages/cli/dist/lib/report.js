...
export function generateMarkdownReport(analysis) {
    ...
    if (pools.length > 0) {
        ...
        lines.push('|---|------|-----|-----|-----|---------|-------|-----|');
        pools.forEach((p, i) => {
            lines.push(`| ${i + 1} | ${p.name || p.address.slice(0, 10)} | ${p.dex || '-'} | ${p.version} | ${fmtUsd(p.tvl || 0)} | ${fmtUsd(p.volume24h)} | ${fmtPrice(p.priceUSD)} | ${p.fee || '-'} |`);
        });
        lines.push('');
    }
    // Spread analysis
...
// JSON Report
// ═══════════════════════════════════════════════════════════════════════════════
export function generateJsonReport(analysis) {
    return JSON.stringify(analysis, null, 2);
}
// ═══════════════════════════════════════════════════════════════════════════════
// Empty report (0 pools)
// ═══════════════════════════════════════════════════════════════════════════════
export function generateEmptyReport(tokenAddress, symbol) {
    const lines = [];
