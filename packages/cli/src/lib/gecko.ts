...
export async function fetchPoolsForToken(
  ...
  for (let page = 1; page <= 3; page++) {
    ...
    if (verbose) console.error(`  Fetching page ${page}: ${url}`);
    ...
    for (const pool of pageData) {
      ...
      if (minTvl > 0 && (tvl === null || tvl < minTvl)) continue;

      let version = 'V2';
      const dexName: string = attrs.dex || '';
      ...
      const quoteTokenId: string = rels.quote_token?.data?.id || '';
      ...
      const nameParts = (attrs.name || '').split('/').map((s: string) => s.trim());
      ...
      const isTargetBase =
        baseTokenAddress.toLowerCase() === address.toLowerCase();
